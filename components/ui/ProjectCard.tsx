...
export function ProjectCard({
  ...
  imageAlt,
  imagePosition = "right",
  imageContainerClassName = "transparent",
}: ProjectCardProps) {
  ...
  return (
    <article
      ref={ref}
      className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-32 items-center mb-32 last:mb-0 fade-up"
    >
      <div
        className={`flex flex-col ${isImageRight ? "lg:pr-10" : "lg:order-2 lg:pl-10"}`}
      >
        <span
      ...
      <div
        className={`flex items-stretch gap-6 lg:gap-12 w-full ${isImageRight ? "" : "lg:order-1 flex-row-reverse"}`}
      >
        {/* Main Full Image */}
        <div
          className={`aspect-square relative w-[85%] shrink-0 overflow-hidden transition-transform duration-700 hover:scale-[1.02] ${imageContainerClassName}`}
        >
