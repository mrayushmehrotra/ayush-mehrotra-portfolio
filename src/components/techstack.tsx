      ...
      <div className="h-fit p-4 md:p-8 space-y-8">
        {Object.entries(memoizedStackData).map(([category, items], index) => (
          ...
          >
            ...
            <div className="w-full z-50  bg-zinc-900/90 rounded-xl  md:w-[50%] grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 p-4 md:p-8    backdrop-blur-sm">
              {items.map((item: StackItem) => (
                <motion.div
                  drag
                  key={item.id}
                  dragConstraints={{ left: 2, right: 20, top: 0, bottom: 0 }}
                  className="relative border-white  p-[1px] overflow-hidden rounded-md group"
                  whileHover={{ scale: 0.95 }}
                >
                  <div className="absolute -inset-[2px] rounded-lg bg-gradient-to-r from-[#f1c830]    via-zinc-500  to-[#F1C830] animate-moving-background bg-[length:200%_200%]" />
