...
const getField = (project: any, field: string, fallback: string) => project[field] || fallback;
      ...
      {Projects.map((project) => (
        <div key={project.title} className="w-full">
          <div className="relative w-full">
            {/* Animated white background wipe */}
            <motion.div
              initial={{ height: 0 }}
            ...
            <motion.div
              layout
              initial={false}
              className={`flex w-full items-center transition-all duration-700 cursor-pointer relative z-10 border-t border-b border-white
            ...
            >
              <p className="flex-1 font-semibold py-3 px-2 transition-colors duration-500">{project.title}</p>
              <p className="flex-1 py-3 px-2 transition-colors duration-75">{getField(project, "category", "Web App")}</p>
              <p className="flex-1 py-3 px-2 transition-colors duration-75">{getField(project, "client", "Personal")}</p>
              <p className="flex-1 py-3 px-2 transition-colors duration-75">{getField(project, "year", "2024")}</p>
              <p className="flex-1 py-3 px-2 transition-colors duration-75">
                <a href={project.link} target="_blank" rel="noopener noreferrer" className="underline text-blue-400 hover:text-blue-600">Visit</a>
              </p>
            </motion.div>
          </div>
          {/* Details Panel */}
            ...
            {selected === project.title && (
              ...
              >
                ...
                <div className="flex flex-col justify-center p-4 text-white">
                  ...
                  <p><span className="font-semibold text-blue-400">Category:</span> {getField(project, "category", "Web App")}</p>
                  <p><span className="font-semibold text-blue-400">Client:</span> {getField(project, "client", "Personal")}</p>
                  <p><span className="font-semibold text-blue-400">Year:</span> {getField(project, "year", "2024")}</p>
                  <a href={project.link} target="_blank" rel="noopener noreferrer" className="text-blue-400 underline mt-2">Visit Project</a>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
