...
const Project = () => {
  ...
  // Calculate yOffsets for all projects at the top level
  const yOffsets = Projects.map((_, index) => {
    const start = index / n;
    const end = (index + 1) / n;
        ...
        <div className="relative w-[80%] h-[70vh]">
          {Projects.map((item, index) => (
            <motion.div
              key={item.id}
              style={{
                y: yOffsets[index], // Use the pre-calculated offset
                zIndex: n + index, // Changed to n - index for proper stacking
              }}
              ...
              <div className="w-full md:w-1/2 p-4 sm:p-6 md:p-8 text-white flex flex-col justify-center bg-black order-2 md:order-1">
                ...
                <div className="flex flex-wrap gap-2 mt-4 sm:mt-6">
                  {item.techStack.map((tech, i) => (
                    <span
                      key={i}
                      className="bg-white font-bold text-black text-sm sm:text-xs py-2 px-4 rounded-full"
                    >
                      {tech}
                    </span>
                  ))}
